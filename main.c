...
//                | MSC_READCTRL_BUSSTRATEGY_CPU  // у CPU приоритет обращния к матрице шин
  ...
  while(1)
  {
    ...
    if( sinxro ) 
    {
...
#endif
        if( color[0][0].g ) --color[0][0].g;  else
        if( color[0][0].r ) --color[0][0].r;  else
        if( color[0][0].b ) --color[0][0].b;  else
        {
        	color[0][0].g = 0x60;
        	color[0][0].r = 0x60;
        	color[0][0].b = 0x30;
        }
        if( color[1][0].r ) --color[1][0].r;  else
        if( color[1][0].b ) --color[1][0].b;  else
        if( color[1][0].g ) --color[1][0].g;  else
        {
          color[1][0].g = 0x11;
          color[1][0].r = 0x40;
          color[1][0].b = 0x40;
        ...
        if( color[2][0].r ) ++color[2][0].r;  else
        if( color[2][0].g ) ++color[2][0].g;  else
        if( color[2][0].b ) ++color[2][0].b;  else
        {
          color[2][0].g = 0x1;
          color[2][0].r = 0x1;
          color[2][0].b = 0x31;
        }
        if( color[3][0].b ) ++color[3][0].b;  else
        if( color[3][0].g ) ++color[3][0].g;  else
        if( color[3][0].r ) ++color[3][0].r;  else
        {
          color[3][0].g = 0x1;
          color[3][0].r = 0x31;
          color[3][0].b = 0x1;
...
#ifdef DEBUG
        color[TAPE_LENGHT-1][0].g = 0x55;
        color[TAPE_LENGHT-1][0].r = 0x05;
        color[TAPE_LENGHT-1][0].b = 0x0A;
#endif
        ColorToRAW();
        sinxro=0;
#ifdef DEBUG
...
#endif
		for(shi=0x00800000, bits=0; bits<24; bits++ ) // цикл по битам диода
		{
