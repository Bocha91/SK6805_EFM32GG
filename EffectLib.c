...
int  accumulating_1_RGB_run(uint8_t tape, uint32_t speed) // speed 1 -очень быстро 100 медленно 200 ещё медленнее
{
    ...
    // Переписываею ленту со cдвигом на 3 светодиода в сторону дальнего конца
...
// несколько меняющихся центров зарождения с шагом 1
int  accumulating_n_RGB_run(uint8_t tape, uint32_t speed) // speed 1 -очень быстро 100 медленно 200 ещё медленнее
{
    ...
    if(start == 0)
    {
      ...
      dels = ost((dels<20)?dels:20); // новое число точек зарождения
...
//-------------------------------------------------------------------
// несколько меняющихся центров зарождения с переменным шагом 
int  accumulating_n2_RGB_run(uint8_t tape, uint32_t speed) // speed 1 -очень быстро 100 медленно 200 ещё медленнее
{
    static int count = 0;
    static uint32_t save[20];
    static int clk = 0;
...
st_accumulating_n2_RGB st_accumulating_n2_RGB_init(uint8_t tape, uint32_t speed) // speed 1 -очень быстро 100 медленно 200 ещё медленнее
{
    ...
    //save[20];
    this.clk = 0;
    this.dels = sqrt(TAPE_LENGHT); // число точек зарождения
    //LENS;  // кусочек (его длина)
    this.n2=1;  // шаг удлинения
    this.start = 0;
    return this;
}
